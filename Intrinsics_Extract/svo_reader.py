...
class SVOReader:
    def __init__(self, filepath, serial_number):
        ...
        # Initialize Camera (only to read metadata from .svo, no physical camera needed)
        self._cam = sl.Camera()
        ...
        if status != sl.ERROR_CODE.SUCCESS:
            print("Error opening SVO file: ", status)


    def get_camera_intrinsics(self):
        # Extract calibration parameters from the .svo file
        calib_params = self._cam.get_camera_information().camera_configuration.calibration_parameters
